...
+ Лист с кодами (содержит `код` в названии)
  ...
  + Пол (М / Ж) участника (столбец с `пол`)
  + Шифр (столбец с `код`)
...
### Загрузка списка школьников (`ExcelFullReader`, `qtype=2`)
...
+ Лист с результатами (содержит в `результаты` названии)
  + Шифр (столбец с `шифр`)
  + Первичный балл (столбец с `балл`) 
### Выгрузка предмета (```py # TODO: описание```)
...
### Выгрузка кодов школьников (```py # TODO: описание```)
+ Лист с кодами (`Кодировка`)
  + `Фамилия`
  + `Имя`
  + `Класс` (буква и цифра)
  + `Код`
### Выгрузка результатов классов (```py # TODO: описание```)
+ Лист `Общий` и отдельные листы на классы (`7 класс`, например)
  + `Место`
  + `Класс` (буква и цифра)
  + `Сумма`
  + `Не 0`
...
### Выгрузка всего года (`ExcelFullWriter`)
